...
import { LIVING_SITUATION_OPTIONS, PLAN_5 } from '@utils/constants'
...
const CombinedOnboarding: React.FC<CombinedOnboardingProps> = ({ onSubmit, loading = false }) => {
  ...
  // Use Plan 5 for all new students (2026 onwards)
  const planConfig = PLAN_5
  ...
  // Calculate loan amounts using Plan 5
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 sm:space-y-6">
        ...
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-900 mb-2">
            <strong>ℹ️ Note:</strong> We calculate the total loan based on tuition fees (£{Math.round(annualTuition)}/year for 2026-27) plus your eligible maintenance allowance, minus your parental contribution. This uses UK Plan 5 repayment terms (threshold: £25,000, 9% repayment rate, RPI interest, 40-year forgiveness).
          </p>
          <p className="text-xs text-blue-800">
            Next, we'll show you 4 salary scenarios (£30k-£60k) with expected monthly repayments based on Plan 5 rules.
          </p>
        </div>
